...
public class OddsFetcher
{
    ...
    public OddsFetcher(HttpClient httpClient, string baseUrl, string apiKey)
    {
        _http = httpClient;
    ...
    /// <summary>
    /// Descarga el snapshot historico de lineas Totals (Over/Under) de The Odds
    /// API (endpoint historico /v4/historical/sports/baseball_mlb/odds) para la
    /// fecha indicada. Devuelve una cotizacion por (evento, casa de apuestas).
    /// </summary>
    public async Task<List<LineaOdds>> ObtenerLineasHistoricasAsync(
        DateTime fecha, TimeSpan horaSnapshotUtc)
    {
        var fechaSnapshot = new DateTimeOffset(
            DateTime.SpecifyKind(fecha.Date.Add(horaSnapshotUtc), DateTimeKind.Utc));
        string url = $"{_baseUrl}/baseball_mlb/odds" +
                     $"?apiKey={Uri.EscapeDataString(_apiKey)}" +
                     "&regions=us&markets=totals&oddsFormat=decimal&dateFormat=iso" +
                     $"&date={fechaSnapshot:yyyy-MM-ddTHH:mm:ssZ}";

        string response = await LeerRespuestaAsync(url, "historico");
        using var doc = JsonDocument.Parse(response);

        if (!doc.RootElement.TryGetProperty("data", out var eventos))
        {
            Console.WriteLine("[ODDS] La respuesta historica no contiene 'data'.");
            return new List<LineaOdds>();
        }

        return ParsearEventos(eventos, fecha.Date);
    }

    /// <summary>
    /// Descarga las lineas Totals ACTUALES (endpoint gratuito) como snapshot.
    /// No requiere plan de pago: permite acumular lineas reales dia a dia.
    /// </summary>
    public async Task<List<LineaOdds>> ObtenerLineasActualesAsync()
    {
        ...
        string response = await LeerRespuestaAsync(url, "actual");
        using var doc = JsonDocument.Parse(response);

        if (doc.RootElement.ValueKind != JsonValueKind.Array)
        {
            Console.WriteLine("[ODDS] La respuesta actual no es un arreglo.");
            return new List<LineaOdds>();
        }

        return ParsearEventos(doc.RootElement, DateTime.Today);
    }

    private async Task<string> LeerRespuestaAsync(string url, string tipo)
    {
        ...
        if (!respuesta.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException(
                $"The Odds API ({tipo}): HTTP {(int)respuesta.StatusCode} "
                + $"{(string.IsNullOrWhiteSpace(detalle) ? respuesta.StatusCode : detalle)}");
        }
        return cuerpo;
    }

    private List<LineaOdds> ParsearEventos(JsonElement eventos, DateTime fechaPorDefecto)
    {
        var lineas = new List<LineaOdds>();
        foreach (var evento in eventos.EnumerateArray())
        {
            ...
            foreach (var casa in casas.EnumerateArray())
            {
                ...
                foreach (var mercado in mercados.EnumerateArray())
                {
                    ...
                    if (!keyMercado.Equals("totals", StringComparison.OrdinalIgnoreCase))
                        continue;
                    ...
                    decimal? linea = null, cuotaOver = null, cuotaUnder = null;
                    foreach (var resultado in resultados.EnumerateArray())
                    {
                        ...
                        if (nombre.Equals("Over", StringComparison.OrdinalIgnoreCase))
                        {
                            cuotaOver = precio;
                            linea = LeerDecimal(resultado, "point") ?? linea;
                        }
                        else if (nombre.Equals("Under", StringComparison.OrdinalIgnoreCase))
                        {
                            cuotaUnder = precio;
                        }
                    }

                    if (linea is null || cuotaOver is null || cuotaUnder is null)
                        continue;

                    lineas.Add(new LineaOdds(
                        eventoId, casaKey, fechaLocal, home, away, commenceUtc,
                        linea.Value, cuotaOver.Value, cuotaUnder.Value,
                        ultimaActualizacion));
                }
            }
        }
